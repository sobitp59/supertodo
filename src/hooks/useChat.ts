...
export function useChat() {
  ...
  const { chatSessions, activeChatSessionId, addChatMessage, createChatSession, settings } = useStore();
  ...
  }, [chatHistory, activeChatSessionId, settings.ai, addChatMessage, createChatSession]);
...
async function executeActions(rawActions: any[]): Promise<ChatAction[]> {
  ...
  for (const raw of rawActions) {
    ...
    const action: ChatAction = { action: raw.action, params: raw.params, success: false, resultText: '' };
      ...
      switch (raw.action) {
        case 'add_todo': {
          ...
          if (text) {
            ...
            if (catId) { state.addTodo(catId, text, todoDate); action.success = true; action.resultText = `Added todo: "${text}" for ${todoDate}`; }
          }
          break;
        }
        case 'complete_todo': {
          ...
          if (todo && !todo.completed) { state.toggleTodo(id); action.success = true; action.resultText = `Completed: "${todo.text}"`; }
          else if (todo?.completed) { action.success = true; action.resultText = `Already completed: "${todo.text}"`; }
          break;
        }
        case 'add_note': {
          ...
          if (title) {
            ...
            if (catId) { state.addNote(catId, title); action.success = true; action.resultText = `Created note: "${title}"`; }
          }
          break;
        }
        case 'update_goal_progress': {
          ...
          if (goal && typeof progress === 'number') { state.updateYearlyGoal(id, { progress: Math.min(100, Math.max(0, progress)) }); action.success = true; action.resultText = `Updated "${goal.title}" to ${progress}%`; }
          break;
        }
        case 'update_job_status': {
          const { id, status } = raw.params;
          const job = state.jobApplications.find(j => j.id === id);
          if (job && status) { state.updateJobApplication(id, { status }); action.success = true; action.resultText = `Updated ${job.company} to "${status}"`; }
          break;
        }
        case 'add_challenge_entry': {
          const { challengeId, date, status } = raw.params;
          const challenge = state.challenges.find(c => c.id === challengeId);
          if (challenge && date && status) { state.setChallengeDay(challengeId, date, status); action.success = true; action.resultText = `Marked ${challenge.emoji} "${challenge.name}" as ${status}`; }
          break;
        }
        default: action.resultText = `Unknown action: ${raw.action}`;
      }
    } catch (err: any) { action.success = false; action.resultText = `Failed: ${err.message}`; }

    if (!action.resultText) action.resultText = `${raw.action} — no matching data found`;
    results.push(action);
  }
  return results;
}
